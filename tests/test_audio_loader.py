...
def test_output_is_float32(mock_load: object) -> None:
    mock_load.return_value = (np.zeros(CLAP_SAMPLE_RATE, dtype=np.float64), CLAP_SAMPLE_RATE)  # type: ignore[attr-defined]
...
def test_librosa_is_called_with_correct_sample_rate_and_mono(mock_load: object) -> None:
    ...
    mock_load.assert_called_once_with(str(Path("audio.wav")), sr=CLAP_SAMPLE_RATE, mono=True)  # type: ignore[attr-defined]
